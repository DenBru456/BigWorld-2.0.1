...
float3 fakeLight1Direction
<
	string UIName = "Fake Light 1 Direction";
	string UIDesc = "Light 1 direction for the fake triangle lighting setup";
	bool artistEditable = true;
> = {-0.2,0.2,-1};

float3 fakeLight1Colour
<
	string UIName = "Fake Light 1 Colour";
	string UIDesc = "Light 1 colour for the fake triangle lighting setup";
	bool artistEditable = true;
> = {1.0,0.9,0.8};

float3 fakeLight2Direction
<
	string UIName = "Fake Light 2 Direction";
	string UIDesc = "Light 2 direction for the fake triangle lighting setup";
	bool artistEditable = true;
> = {1.0,-1.0,-0.1};

float3 fakeLight2Colour
<
	string UIName = "Fake Light 2 Colour";
	string UIDesc = "Light 2 colour for the fake triangle lighting setup";
	bool artistEditable = true;
> = {0.0,0.0,0.5};

float fakeLightAmount
<
	string UIName = "Fake lighting amount";
	string UIDesc = "How much fake lighting should be used when other lights are active";
	float UIMin = 0;
	float UIMax = 2;
	int UIDigits = 1;
	bool artistEditable = true;
...
// Point lights
float3 pointLightAtt( in float3 position, in float3 normal, in PointLight light, out float amt )
{
	...
	att = saturate(att);
	amt = att.x * 0.33 * dot(light.colour.rgb,light.colour.rgb);
...
BW_COLOUR_OUT ps_main_3_0( Output input )
{
...
#endif
	float4 specularMap = tex2D( specularSampler, input.tc );
	...
	float totalPointLighting = 0.0;
	for (int i = 0; i < nPointLights; i++)
	{
		float amt;
		diffuse += pointLightAtt( input.worldPos.xyz, normal, pointLights[i], amt );
		totalPointLighting += amt;
	}
	
	// grab the eye vector
	float3 eye = normalize(wsCameraPos - input.worldPos.xyz);
	
	// fake some triangle lighting setup to make him look nice
	totalPointLighting = saturate(totalPointLighting);
	float howMuch = fakeLightAmount * (1.0 - totalPointLighting);
	float3 viewNormal = mul( normal, view );
	float3 fakeLighting = fakeLight1Colour * dot( viewNormal, normalize(fakeLight1Direction) );
	fakeLighting += fakeLight2Colour * dot( viewNormal, normalize(fakeLight2Direction) );
	diffuse += (fakeLighting * howMuch);
	...
	rimLight = saturate( rimLight * attenuation * specularMap.a );
	...
	float4 colour;
	colour.xyz = diffuse + (diffuseMap.xyz*rimLight*rimColour.xyz) + saturate(specular) * specularMap.xyz;
#ifdef MOD2X
				...
				uniform int nSpecPoints ) : COLOR0
{
	...
	// Calculate result colour
	float4 colour;
	colour.xyz = skyMap * diffuse + (rimLight*diffuseMap.xyz*rimColour.xyz) + saturate(specular) * specularMap.xyz;
#ifdef MOD2X
	colour.xyz *= (1 + diffuseLightExtraModulation);
#endif
	colour.w = diffuseMap.w;
