...
OutputVertex vs_main( FloraVertex input )
{
	...
	float4 animatedWorldPos = mul(animatedPos, vertexToWorld);
	...
	o.tcDepth.xy = o.tc0.xy;
	BW_DEPTH(o.tcDepth.z, o.pos.z);
	
	return o;
}

sampler lightMapSampler = BW_SAMPLER(lightMap, CLAMP);
sampler floraMapSampler = BW_SAMPLER(floraTexture, CLAMP);

BW_COLOUR_OUT ps_main( OutputVertex input ) : COLOR
{
	float4 lightMap = tex2D( lightMapSampler, input.tc0 );
	float4 diffuse = tex2D( floraMapSampler, input.tc2 );
	float4 light;
	light.rgb = lightMap + ambient.rgb;
	light.a = input.col.a;
	BW_FINAL_COLOUR( input.tcDepth.z, light * diffuse );
};


technique shader3MRT
{
   pass Pass_0
   {
