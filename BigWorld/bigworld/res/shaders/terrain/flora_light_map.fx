...
// The world transform needed for lighting
float4x4 world;
...
VS_OUTPUT vs_main(const VS_INPUT v)
{
	...
	float4 tempPos = mul( v.pos, lightMapProjection );
	
	// The terrain block may have been rotated, so adjust the screen space 
	// orientation
	o.pos = float4( dot(tempPos.xy, orientationX),
					dot(tempPos.xy, orientationY),
					tempPos.z,
					tempPos.w );
	o.blend = v.blend;	
	...
	o.diffuse.xyz = directionalLight( mul(v.normal, world), directionalLights[0] );
