...
VS_OUTPUT vs_spec(const VS_INPUT v)
{
	...
	float3 worldNormal = mul(v.normal, world);
	
	o.diffuse.xyz = directionalLight( worldNormal, directionalLights[0] );
	...
	o.reflection.xyz = worldNormal.xyz;
	o.view = worldPos.xyz;
#else
	o.reflection.xyz = reflect(directionalLights[0].direction.xyz, worldNormal);	
	o.view = normalize(wsCameraPos.xyz - worldPos.xyz);	
	// Adding a per vertex fresnel
	o.reflection.w = fresnel( -o.view, worldNormal, fresnelExp, fresnelConstant );	
...
VS_OUTPUT11WITHFIX vs_spec_11(const VS_INPUT v)
{
	...
	o.reflection = reflect(directionalLights[0].direction.xyz, worldNormal);	
