...
float2 orientationX = float2(1,0);
float2 orientationY = float2(0,1);
...
TerrainVertexOutput vs30_terrain(const TerrainVertex vertex)
{
	...
	float2 screenPos = float2( vertex.xz.x * 2.0 - 1.0, vertex.xz.y * -2.0 + 1.0 );
	
	// The terrain block may have been rotated, so adjust the screen space 
	// orientation
	oVertex.position = float4( dot( screenPos, orientationX), dot( screenPos, orientationY), 0.1, 1 );
	
	// Calculate the world position for use with lighting
...
float4 ps30_terrain( const TerrainVertexOutput terrainFragment ) : COLOR0
{
	...
	float3 normal = mul(terrainNormal( normalMapSampler, terrainFragment.normalUV), world );
