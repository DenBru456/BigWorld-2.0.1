...
float softParticleContrast = 2.0;	//power of contrast fn
...
PS_FF_INPUT_RT vs_ff( VertexXYZDUV2 i )
{
	...
	// u = 0.5 * projPos.x / projPos.w + 0.5;
	// v = 0.5 * -projPos.y / projPos.w + 0.5;
...
#define WATER_TEX_COORDS_3_0 \
	...
	float c = 1.0;\
	float depth = 0;\
	float softIntersect = 1.0;\
	if ( !bypassDepth )\
	{\
		c= Contrast(zdiff, softParticleContrast);\
		if( (zdiff * c) <= EPSILONZ )\
		{\
			discard;\
			return col;\
