...
float2 sunScreenPos()
{
	float3 sunWorldPos = eyePos - directionalLights[0].direction * farPlane.x;
...
float4 sunColour( float2 sunPos, PS_INPUT input ) : COLOR0
{
	...
	return float4( len, len, len, 1.0 ) * directionalLights[0].colour;
}
...
float4 ps_main( PS_INPUT input ) : COLOR0
{
	...
	clip( dot(input.worldNormal, -directionalLights[0].direction) );
