...
Output vs_main( VS_INPUT input )
{
	...
	float hw = screenDimensions.z;
	float hh = screenDimensions.w;
	
	// Transform vertex to render target (screen space)
	float2 SCREEN_FACTOR_OFFSET;
	SCREEN_FACTOR_OFFSET.x = -1.0 / width;
	SCREEN_FACTOR_OFFSET.y = 1.0 / height;
	o.pos.xy = input.pos.xy + SCREEN_FACTOR_OFFSET.xy;	
	o.pos.zw = 1.0;
	...
	float2 UVFIX = float2( uFixup / width, vFixup / height );

	float t = time * (speed/10.f);
	float4 ANIMATION = float4( spreadX / width, spreadY / height, t, twoPi );
