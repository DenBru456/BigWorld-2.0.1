...
{
	return ((this.procField.value.length > 0) &&
			(this.pathField.value.length > 0));
};
...
PageManager.prototype.onDBFetchSuccess = function ( res )
{
	...
	newFilterOption = OPTION( {value:"-1", class:"primary_option_special",
								selected:true}, "(new)");
	...
	{
		headingOption = OPTION( {value:"-2", class:"primary_option_title",
									disabled:true},
								"Pre-Configured");
	...
	for (var i in res.globalFilters)
	{
		...
		filters.push( OPTION( {value:globalCounter, class:"primary_option"},
						dbEntry.name) );
		globalCounter++;
	}
	...
	if (res.userFilters.length > 0)
	{
		...
		for (var i in res.userFilters)
		{
			...
			userOption = OPTION( {value:globalCounter, class:"primary_option"},
							dbEntry.name);
