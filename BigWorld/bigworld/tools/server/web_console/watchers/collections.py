...
class Collections( turbogears.controllers.Controller ):
	...
	def view( self, name=None ):
		...
		if not name:
			return self.error( "No watcher collection name provided." )

		try:
			collection = getCollection( name )
		except:
			return self.error( "No watcher collection '%s' known." % name )
	...
	@identity.require( identity.not_anonymous() )
	@ajax.expose
	def createCollection( self, user, name ):
		c = cluster.cache.get()
		user = c.getUser( user )

		cluster_model.ccCustomWatchers( user = util.getSessionUID(),
							  pageName = name)
