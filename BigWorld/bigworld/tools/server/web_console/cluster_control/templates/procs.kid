...
<div py:def="pageContent()">
	...
	<script type="text/javascript">
		...
		var isProduction = ${str( isProduction ).lower()};
	</script>
	...
	<p>
		<span py:if="user.getServerProcs() ">
			...
			<a href="${tg.url( 'restart', user=user.name )}"
			 	onclick="return !isProduction || confirm( 'Are you sure you want to restart the server?' );">
				Restart the server</a>
			|
			<a py:if="areWorldProcessesRunning"
			   onclick="return !isProduction || confirm( 'Are you sure you want to stop the server?' );"
			   href="${tg.url( 'stop', user=user.name )}">
				Stop the server</a>

			<a py:if="not areWorldProcessesRunning" 
			   onclick="return !isProduction || confirm( 'Are you sure you want to kill the server?' );"
	...
	<span py:if="layoutErrors" class="alert">
	Server Layout Warnings:
	</span>
	<ul py:if="layoutErrors">
