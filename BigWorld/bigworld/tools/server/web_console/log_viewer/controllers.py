...
from web_console.common import encoding
...
bwlog = __import__( "bin.%s.bwlog" % pycommon.util.MF_CONFIG(), globals(), locals(), ['bwlog'] )
...
class LogViewer( module.Module ):
	...
	def fetch( self,
		...
		if encoding.ML_ENCODING != "utf-8":
			message = message.decode( "utf-8" ).encode( encoding.ML_ENCODING )
			exclude = exclude.decode( "utf-8" ).encode( encoding.ML_ENCODING )
	...
	def runQuery( self, query, showMask, live, context, maxLines, _async_ ):
		...
		while True:
			...
			for result in query:
				...
				line = result.format( showMask )
				try:
					line = line.decode( encoding.ML_ENCODING )
				except:
					line = repr( line[:-1] )[1:-1] + '\n'
