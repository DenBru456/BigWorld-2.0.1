...
/* namespace */ var OutputPane =
{
	...
	OutputPane: function( anchorElt, params )
	{
		...
		appendChildNodes( this.table,
						  TR( {style: "vertical-align: top"},
							  ...
							  TD( {width: "90px", align: "right"},
								  this.statusimg, A( {}, this.stop ),
								  this.spinner ) ),
						  TR( {id: "clockrow"},
							  TD( {colspan: "3"}, this.clock ) ),
		...
		this.fill = function()
		{
			...
			// Magic number of 20 is the padding we see at the RHS of the page
			odim.w = this.width ? this.width : wdim.w - opos.x - 20;
			odim.h = this.height ? this.height : wdim.h - opos.y - 20;
		...
		{
			// Don't display the time row if the server and browser are the same
			if (ServerTime.timezoneOffset == 0)
			{
			...
			this.clock.innerHTML = "Current Server Time: " +
					OutputPane.niceDate( ServerTime.asBrowserTime() );
		}
