...
WC_CRON_HOURLY_FILE = "bw_web_console_access.cron"
...
BWLOG_SO = "../bin/%s/bwlog.so" % util.MF_CONFIG()
TOOLS_SECTION = "tools"
...
OPTIONAL_CONFIG_MAIL_TO = "mail_to"
OPTIONAL_CONFIG_MAIL_FROM = "mail_from"
...
def databaseInstall():
	...
	tgsqluri = "sqlobject.dburi=\"notrans_mysql://%s:%s@%s:%s/bw_web_console?charset=utf8\"\n" \
...
def setSecurityContextType( file, type ):
	"""
	Sets the security context of a file if this system has SELinux.
	"""
	if os.path.isdir( "/etc/selinux/" ):
		...
		if isEnforcing:
			...
			# Security context is in the format 'user:role:type filename'
			context = os.popen( "ls --scontext %s" % file ).readlines()[0]
			context = context.replace( file, "" ).strip()
			...
			print "\n+ Chcon'ing '%s' type to %s" % (file, type)
			if os.system( "chcon -t %s %s" % (type, file) ) != 0:
...
def readConfigs( configs, shouldReadOptionalConfigOnly ):
	"""
	Reads optional configuration options that may be commented out.
	These configuration options need to be read separately becuase 
	ConfigParser ignores comment lines.

	@param optionConfigs: A dictionary containing optional configuration 
						  options.  
						  Format: { <confi_name>: 
						  	(config_value, <isCommentedOut>) }
	@param shoudlReadOpionalConfigOnly: True if should read optional
										configuration options only.
	"""

	regExpPattern = "([#]*)[ \t]*([a-zA-Z0-9_]+)[ \t]*=[ \t]*(.*)"
	configRegExp = re.compile( regExpPattern )

	try:
		fileDesc = open( CONF_FILE )
	except IOError:
		return

	for line in fileDesc:
		result = configRegExp.match( line )

		if result == None:
			continue

		matchedElements = result.groups()
		if matchedElements[1] == OPTIONAL_CONFIG_MAIL_FROM:
			if len( matchedElements[0] ) == 0: 
				# The configuration option is not commented out.
				configs[OPTIONAL_CONFIG_MAIL_FROM] = \
					(matchedElements[2], False)

			else:
				# The configuration option is commented out.
				configs[OPTIONAL_CONFIG_MAIL_FROM] = \
					(matchedElements[2], True)

		elif matchedElements[1] == OPTIONAL_CONFIG_MAIL_TO:
			if len( matchedElements[0] ) == 0: 
				# The configuration option is not commented out.
				configs[OPTIONAL_CONFIG_MAIL_TO] = \
					(matchedElements[2], False)
			...
			if not shouldReadOptionalConfigOnly:
				configs[matchedElements[1]] = (matchedElements[2], False)	
...
def install():
	...
	# Needed for "mail_to" and "mail_from" configuration options.
	optionalConfigs = {}	
	if not shouldQuery:
		...
		# Only read optional configuration options.  Optional configuration 
		# options are those that can be commented out.
		readConfigs( optionalConfigs, True )
	...
	if shouldQuery:
		...
		# Handles optional configuration options.
		mailFrom = "bwtools@company.com"
		mailTo = "bwtools_monitor@company.com"
		if OPTIONAL_CONFIG_MAIL_FROM not in optionalConfigs :
			optionalConfigs[OPTIONAL_CONFIG_MAIL_FROM] = \
				(mailFrom, True)

		if OPTIONAL_CONFIG_MAIL_TO not in optionalConfigs :
			optionalConfigs[OPTIONAL_CONFIG_MAIL_TO] = \
				(mailTo, True)
	...
	# Make adjustments to optional configuration options.
	# If either mail_to or mail_from is commented out, 
	# then both should be commented out.
	mailFromValue, isMailFromCommentedOut = \
		optionalConfigs[OPTIONAL_CONFIG_MAIL_FROM]
	mailToValue, isMailToCommentedOut = \
		optionalConfigs[OPTIONAL_CONFIG_MAIL_TO]

	if isMailFromCommentedOut or isMailToCommentedOut:
		optionalConfigs[OPTIONAL_CONFIG_MAIL_FROM] = \
			(mailFromValue, True)
		optionalConfigs[OPTIONAL_CONFIG_MAIL_TO] = \
			(mailToValue, True)
	...
	# Re-write CONF_FILE configuration file.  This needs to be done 
	# because ConfigParser class (and subclasses of it) do not support 
	# writing out commented out configuration options.

	# Read in current state of configuration file.
	curConfigs = {}
	readConfigs( curConfigs, False )

	# Re-write configuration file.  Write compulsory configuration 
	# options first.
	fileDesc = open( CONF_FILE, "w" )
	fileDesc.write( "[tools]\n" )
	for key in sorted( curConfigs ):
		if key not in optionalConfigs:
			configValue, isCommentedOut = curConfigs[key]
			fileDesc.write( "%s = %s\n" % (key, configValue) )

	fileDesc.write( "\n" )

	# Write optional configuration options.
	for key in sorted( optionalConfigs ):
		configValue, isCommentedOut = optionalConfigs[key]
		if isCommentedOut:
			key = "#" + key
	...
	# Install the cron script to access the WebConsole every hour
	distro = platform.dist()

	if distro and distro[ 0 ] == "redhat":
		print "+ Installing cron.hourly to prevent MySQL reconnection issue"

		shutil.copy( "./%s" % WC_CRON_HOURLY_FILE,
					"/etc/cron.hourly/%s" % WC_CRON_HOURLY_FILE )
