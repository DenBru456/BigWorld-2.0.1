...
case "$1" in
  ...
  start)
	...
	su $BW_TOOLS_USERNAME -c "./$EXEC --daemon --pid $PIDFILE \
				-o $BW_TOOLS_LOG_DIR/bwlockd.log \
  ...
  install)
  	...
  	if [[ ! -e $INIT_RD_DIR/bw_functions ]]; then
		ln -sf `pwd`/bw_functions.sh $INIT_RD_DIR/bw_functions
	fi
	
  	if [[ -x $INIT_RD_DIR/$SCRIPT_NAME ]]; then
		$INIT_RD_DIR/$SCRIPT_NAME stop
	fi
	
	cp $SOURCE_SCRIPT_NAME $INIT_RD_DIR/$SCRIPT_NAME
