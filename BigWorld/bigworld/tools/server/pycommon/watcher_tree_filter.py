...
class BadPathError( Error ):
	def __init__( self, badPath, querySubPaths, process ):
		...
		self.msg = "Unsuccessful watcher request to %s for:\n%s" % \
...
def _createTreeForProcess( process, subPaths ):
	...
	for subPath in querySubPaths:
		...
		try:
			batchResult = messages.WatcherDataMessage.batchQuery( paths, [process] )
		except ValueError, e:
...
def printTreeAsTable( tree, depth ):
	...
	if depth == 2:
		...
		print '	'.join( str( value[0] ) for value in tree.values() )
