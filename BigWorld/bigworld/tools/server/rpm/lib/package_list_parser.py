...
excludePathPatterns: Specify an exclusion pattern. The arg string is a regular
expression against which the source path for all files specified using add or
addConfig are searched against, a match means that the file is excluded.
...
macroExpander['mf_config'] = ...
...

for packageFile in parser.fileList( mfRoot, macroExpander ):
...
from package_file import PackageFile, ConfigPackageFile, \
		 DirPackageFile, NewDirPackageFile, LinkPackageFile
...
class PackageListParser( object ):
	...
	def _parseAdd( self, line, lineNum, packageFileClass = PackageFile,
		   hasSource = True ):
		...
		self.packageFiles.append( packageFileClass( lineNum, source,
					destination, mode, owner, group ) )

	@parseCommand( PARSE_COMMANDS, "addConfig" )
	def _parseAddConfig( self, line, lineNum ):
		self._parseAdd( line, lineNum, packageFileClass = ConfigPackageFile )

	@parseCommand( PARSE_COMMANDS, "excludePathPatterns" )
	def _parseExcludePathPatterns( self, line, lineNum ):
		pattern = re.compile( line )
		self.excludePathPatterns.append( pattern )

	@parseCommand( PARSE_COMMANDS, "link" )
	def _parseLink( self, line, lineNum ):
		self._parseAdd( line, lineNum, packageFileClass = LinkPackageFile )

	@parseCommand( PARSE_COMMANDS, "dir" )
	def _parseLink( self, line, lineNum ):
		self._parseAdd( line, lineNum, packageFileClass = NewDirPackageFile,
		 		hasSource = False )

	@parseCommand( PARSE_COMMANDS, "filter" )
	def _filterFile( self, line, lineNum ):
	...
	def _parseCommands( self, lines ):
		...
		for i, line in enumerate( lines ):
			...
			if cmd in PackageListParser.PARSE_COMMANDS.keys():
				...
				try:
					parseCommand( self, args.strip(), lineNum )
				except Exception, e:
	...
	def _shouldExclude( self, path ):
		if path is not None:
			for pattern in self.excludePathPatterns:
				if pattern.search( path ):
					return True
		return False


	def fileList( self, mfRoot, macroExpander ):
		"""
		...
		@param mfRoot 			The top level directory for the source relative
								paths.
			...
			if not packageFile.isCopy():
				...
				for path, dirs, files in os.walk( source ):
					...
					for dirPath in dirs:
						...
						childPackageFile = packageFile.clone( DirPackageFile )
						childPackageFile.source = fullSourcePath
						childPackageFile.destination = \
