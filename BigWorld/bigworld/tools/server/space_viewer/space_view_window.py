...
from pycommon.cluster_constants import MAX_SERVER_CPU
...
class SpaceViewWindow( wx.Frame ):
	...
	def __init__( self, parent, replayer, stopLoggerOnExit = True ):
		...
		helpMenu = wx.Menu()
		helpMenu.Append( self.ID_HELP, "Help\tF1" )
		helpMenu.Append( self.ID_HELP_QUICK, "Quick Reference" )
		...
		wx.EVT_MENU( self, self.ID_HELP, self.onMouseControlsHelp )
		wx.EVT_MENU( self, self.ID_HELP_QUICK, self.onMouseControlsHelp )
	...
	def onMouseControlsHelp( self, event ):
		...
		if event.GetId() == self.ID_HELP:
			helpctrl.DisplayContents()
		elif event.GetId() == self.ID_HELP_QUICK:
			helpctrl.Display( "quick.html" )
...
class SpaceViewSurface( BufferedWindow ):
	...
	def onMMouseUp( self, event ):
		...
		if self.mode == self.ZOOM_SELECTING:
			...
			self.statusBar.SetStatusText( "Pos: %.1f, %.1f" %
