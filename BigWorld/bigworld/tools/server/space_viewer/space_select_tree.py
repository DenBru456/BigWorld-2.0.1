	...
	def onExpanding( self, event ):
		...
		for (uid, proc) in self.cellappmgrs.items():
			...
			if user.name == self.GetItemText( event.GetItem() ):
				...
				children = spaces.getChildren()
				if children == None:
					wx.MessageBox( "Unable to discover server processes.\n\n"
									"Please check server processes are alive "
									"or use --udpWatchers on startup.",
								"Error", wx.ICON_ERROR )
				else:
					for wd in children:
