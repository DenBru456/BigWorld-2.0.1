...
class WorldEditorDirector:
	...
	def showBSPMsg( self, state ):
		if state == 1:
			WorldEditor.addCommentaryMsg( "`SCRIPT/WORLD_EDITOR_DIRECTOR_PY/DRAW_CUSTOM_BSP" )
		else:
			WorldEditor.addCommentaryMsg( "`SCRIPT/WORLD_EDITOR_DIRECTOR_PY/NORMAL_DRAW_MODE" )

	def ownKeyEvent( self, key, modifiers ):
	
		if WorldEditor.tool() == self.heightTool and key == KEY_LSHIFT:
			WorldEditor.setToolMode( "TerrainFilter" )				
		elif WorldEditor.tool() == self.filterTool and key == KEY_LSHIFT:
			WorldEditor.setToolMode( "TerrainHeight" )	
							
		if WorldEditor.tool() == self.holeTool and key == KEY_LSHIFT:
	...
	def onKeyEvent( self, event ):
		...
		if not handled and event.isKeyDown():
			handled = self.ownKeyEvent( event.key, event.modifiers )
