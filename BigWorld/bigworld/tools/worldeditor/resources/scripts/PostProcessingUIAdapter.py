...
def renameChain( newName ):
	...
	if not isNameValid( newName ):
		...
		oldChain = g_currentChain
		g_currentChain = g_chainsFolder + newName + CHAIN_EXT
		ResMgr.rename( oldChain, g_currentChain )
...
def duplicateChain():
	...
	if g_currentChain != "":
		oldChain = g_currentChain
		generateChainName( nameOnly( oldChain ) )
		ResMgr.copy( oldChain, g_currentChain )
	else:
		generateChainName( "default chain" )
...
def getNewChainName( defaultStr = "" ):
	...
	g_currentChain = g_chainsFolder + inputStr	
	return True
...
def generateChainName( oldName ):
	...
	numStr = "%02d" % (maxIdx + 1)
	if pos > 0:
		g_currentChain = g_chainsFolder + oldName[ : pos + len( postfix )]  + numStr + CHAIN_EXT
	else:
		g_currentChain = g_chainsFolder + oldName + postfix + numStr + CHAIN_EXT
