...
(
	...
	-- Known Bugs:
	-- None	
	include "BigWorld_CoPlanar_Check.ms"
	...
	(
		...
		if stdPort == true and heavPort == false and exPort == false then -- is standard portal - Now obsolete, tested b4 planar test
		(
			setUserProp selected "heaven" "false"
			setUserProp selected "exit" "false"
			setUserProp selected "portal" "false"
			
			selected.material = standard()
			selected.wirecolor = color 128 128 128
		)	
		else -- is other portal type or false
		(
