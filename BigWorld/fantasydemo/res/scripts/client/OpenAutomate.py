...
#Just for testing (allowing to run min spec twice)
		...
		#add the urban and highlands test space only on high spec machines
		if os.environ.keys().count("LOW_SPEC") == 0:
			self.tests["RunProfilerUrban"] = (FantasyDemo.runProfiler, ["spaces/urban", "urbanOutput", False],True)
	...
	def flyThroughModeActivatedInternal( activated, resultList ):
		if not activated:
			...
			if shouldRerunTest:
				...
				yield BWWaitForCoroutine( FantasyDemo.handleDisconnectionFromServer() )
...
def setupOpenAutomateMainLoop(delay, testEnded, runEnded):
	...
	if runEnded:
		...
		yield BWWaitForCoroutine(FantasyDemo.setMainMenuActive( False ))
...
def _enumerateOpenAutomateOptions(enumCurrentOptions) :
	...
	#add the video modes
	if enumCurrentOptions:
		modes, current = FantasyDemo.enumVideoModes()
		OpenAutomateWrapper.bwOAAddOptionValue(OPTION_VIDEO_MODE, "TYPE_ENUM", modes[current][0])
	else:
		oaNamedOptionWrapper = OpenAutomateWrapper.CreateOANamedOptionWrapper() 
		oaNamedOptionWrapper.dataType = "TYPE_ENUM"
		...
		modes, current = FantasyDemo.enumVideoModes()
		for mode in modes:
			oaNamedOptionWrapper.enumValue = mode[0]
...
def _setOptions() :
	...
	while setOption != None:
		...
		if setOption.name == OPTION_VIDEO_MODE:
			modes, current = FantasyDemo.enumVideoModes()
			for mode in modes:
				if mode[0] == setOption.enumValue:
					#call the partial function to change the mode
					mode[1]()
