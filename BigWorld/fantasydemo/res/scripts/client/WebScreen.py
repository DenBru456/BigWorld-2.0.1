...
import Keys
import Avatar
from Helpers.PyGUI import LLMozlibKeys
...
class WebScreen( BigWorld.Entity ):
	...
	def onEnterWorld( self, prereqs ):
		...
		usedGraphicsBehaviour = "EFFECT_QUALITY"
		if self.graphicsSettingsBehaviour == WebScreenData.EFFECT_SCROLLING:
			usedGraphicsBehaviour = "EFFECT_SCROLLING"

		self.webPage = BigWorld.WebPageProvider(self.width, self.height,self.dynamic, self.mipmap,u"about:blank", usedGraphicsBehaviour)
		if self.ratePerSecond > 0:
	...
	def hitPot( self, enter, id = None ):		
		...
		if enter:
			...
			self._deactivate()
			self.webPage.navigate("about:blank")
	...
	# --------------------------------------------------------------------------
	def use( self ):
		#Make the WebScreen choosen and inform the Avatar about it.
		if self.interactive:
			player = BigWorld.player()
			...
			self.handleKeyEventInternal(True)
			self.handleKeyEventInternal(False)

	def handleKeyEventInternal( self, isDown ):
	...
	def handleKeyEvent( self, event ):
		...
		if event.key == Keys.KEY_LEFTMOUSE:
			...
			if self.interactive:
				return self.handleKeyEventInternal(event.isKeyDown())
				return True
	...
	def setKeyFocus(self, value):
		if self.mozillaHandlesKeyboard:
			self.webPage.focusBrowser(value, value)
			self.mozillaBrowserFocused = value
		self.webPage.allowCursorInteraction(value)
		
	def setFocus(self, value):
		self.setKeyFocus(value)
		#update the ratePerSecond based on the current focus
		if value:
			self.webPage.ratePerSecond = self.interactiveRatePerSecond
			self.webPage.setDynamic(True)
			GUI.mcursor().automaticCursorManagement = False
		else:
			if self.ratePerSecond > 0:
				self.webPage.ratePerSecond = self.ratePerSecond
			self.webPage.setDynamic(self.dynamic)
			GUI.mcursor().automaticCursorManagement = True
	...
	#mozilla web callbacks
	def onPageChanged( self, event ):
		return True

	def onNavigateBegin( self, event ):
		return True

	def onNavigateComplete( self, event ):
		return True
	...
	def onClickLinkHref( self, event ):
		return True

	def onClickLinkNoFollow( self, event ):
		return True

	def addObserver(self):
		self.webPage.listener = self
		
	def removeObserver(self):
		self.webPage.listener = None
