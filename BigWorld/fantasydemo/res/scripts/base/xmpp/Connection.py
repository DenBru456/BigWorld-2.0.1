...
class Connection( object ):
	...
	def doConnect( self ):
		if not self.username or not self.password:
			print "XMPPConnection: Unable to connect, " \
		...
		(self.hostname, self.resolvedHost, self.port) = XMPPService.details()
	...
	def disconnect( self ):
		if not self.socket:
			print "XMPPConnection: Unable to disconnect, " \
	...
	def onStateAuthenticating( self, stanza ):
		...
		if stanzaName == 'challenge':
			...
			print "Unhandled stanza name:", stanzaName
			print stanza
...
#			return
		...
		if not self.parser:
			print "xmpp.Connection::onXmppEvent: No parser available."
			return

		data = self.socket.recv( RECV_BUFFER_SIZE )
		if not data:
			print "xmpp.Connection::onXmppEvent: Disconnecting from " \
	...
	def handleStanza( self, stanza ):
		...
		# STATE: Registering
			...
			if stanzaName == 'iq' and getAttr( stanza, 'type' ) == 'result':
				...
				print "XMPPConnection::handleStanza: An error was " \
		...
		# STATE: Online
			...
			### error, unknown state
			print "XMPPConnection::handleStanza: Unknown state!"
			pass
	...
	def onIQStanza_RosterItemAdd( self, stanza ):
		...
		if not self.stanzaIDs.has_key( stanzaID ):
			print "Unknown roster addition message received:", stanzaID
			return False
	...
	def onIQStanza_RosterItemDelete( self, stanza ):
		...
		if not self.stanzaIDs.has_key( stanzaID ):
			print "Unknown roster deletion message received:", stanzaID
			return False
	...
	def onIQStanza_LegacyRegister( self, stanza ):
		...
		if not self.stanzaIDs.has_key( stanzaID ):
			print "Received unknown legacy network registration message"
			return False
	...
	def onIQStanza_LegacyDeregister( self, stanza ):
		...
		# Verify that this was a request we submitted
		if not self.stanzaIDs.has_key( stanzaID ):
			print "Received unknown legacy network de-registration message"
			return False
	...
	def onStanzaInfoQuery( self, stanza ):
		...
		# A deletion of a legacy network roster item (post deregistration)
			...
			print "xmpp.Connection::onStanzaInfoQuery: Received unknown " \
					"message ID:", stanzaID
