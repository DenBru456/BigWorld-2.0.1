...
def spawnGuardsInSpace( targetSpace, numberOfGuards, maxGuards, spawnDuration, timeToLive ):
	BigWorld.globalBases[GuardSpawner._globalBaseKey( targetSpace )].spawnGuardsGlobally( numberOfGuards, maxGuards, float( spawnDuration ), float( timeToLive ) )


def removeGuardsFromSpace( targetSpace, numberOfGuards ):
	BigWorld.globalBases[GuardSpawner._globalBaseKey( targetSpace )].removeGuardsGlobally( numberOfGuards )
...
class GuardSpawner( FantasyDemo.Base ):
	...
	def registerGloballyCallback( self, success ):
		try:
			masterGuardSpawner = BigWorld.globalBases[ GuardSpawner._globalBaseKey( self.spaceID ) ]
			...
			ERROR_MSG( "GuardSpawner failed to register or find global base '%s'" % GuardSpawner._globalBaseKey( self.spaceID ) )
	...
	def onTimer( self, controllerID, userData ):
		...
		if userData == GuardSpawner.GLOBAL_REQUEST_SPAWN_TIMER:
			...
			totalSpawnedGuards = sum( [spawnedGuards for spawnedGuards, pendingGuards in self.guardSpawners.values()] )
	...
	def spawnGuardsLocally( self, targetGuardSpawner, numberOfGuards, maxGuards, spawnDuration, timeToLive ):
		masterSpawner = BigWorld.globalBases[GuardSpawner._globalBaseKey( self.spaceID )]
		if self.id != masterSpawner.id:
			masterSpawner.spawnGuardsLocally( targetGuardSpawner, numberOfGuards, maxGuards, spawnDuration, timeToLive )
			return

		controllerID = self.addTimer( 0.5, 0.0, GuardSpawner.LOCAL_REQUEST_SPAWN_TIMER )
		self.localSpawnRequests[ controllerID ] = LocalGuardSpawnRequest( targetGuardSpawner, numberOfGuards, maxGuards, spawnDuration, timeToLive )
		self.recountGuards()


	def spawnGuardsGlobally( self, numberOfGuards, maxGuards, spawnDuration, timeToLive ):
		controllerID = self.addTimer( 0.5, 0.0, GuardSpawner.GLOBAL_REQUEST_SPAWN_TIMER )
		self.globalSpawnRequests[ controllerID ] = GuardSpawnRequest( numberOfGuards, maxGuards, spawnDuration, timeToLive )
		self.recountGuards()


	def removeGuardsGlobally( self, numberOfGuards ):
		controllerID = self.addTimer( 0.5, 0.0, GuardSpawner.GLOBAL_REMOVE_TIMER )
		self.guardRemoveRequests[ controllerID ] = GuardRemoveRequest( numberOfGuards )
		self.recountGuards()
	...
	def queryNumberOfGuards( self, returnMailbox ):
		...
		pendingGuards = sum( [numberOfGuards for numberOfGuards, timeToLive in self.spawnTimers.values()] )

		returnMailbox.notifyNumberOfGuards( self, spawnedGuards, pendingGuards )


	def notifyNumberOfGuards( self, spawnerMailbox, spawnedGuards, pendingGuards ):
	...
	def _initiateLocalGuardSpawns( self, controllerID ):
		...
		totalGuards = sum( [sum([spawnedGuards, pendingGuards]) for spawnedGuards, pendingGuards in self.guardSpawners.values()] )
		maxGuardsToSpawn = max( 0, localSpawnRequest.maxGuards - totalGuards )
		guardsToSpawn = min( localSpawnRequest.numberOfGuards, maxGuardsToSpawn )

		localSpawnRequest.targetGuardSpawner.spawnGuards( guardsToSpawn, localSpawnRequest.spawnDuration, localSpawnRequest.timeToLive )

		for spawner, (spawnedGuards, pendingGuards) in self.guardSpawners.items():
			if spawner.id == localSpawnRequest.targetGuardSpawner.id:
				self.guardSpawners[spawner] = (spawnedGuards, pendingGuards + guardsToSpawn)
	...
	def _initiateGlobalGuardSpawns( self, controllerID ):
		...
		guardsToSpawn = min( globalSpawnRequest.numberOfGuards, maxGuardsToSpawn )

		for spawner, (spawnedGuards, pendingGuards) in self.guardSpawners.items():
			numberOfGuards = guardsToSpawn // len( self.guardSpawners )
			spawner.spawnGuards( numberOfGuards, globalSpawnRequest.spawnDuration, globalSpawnRequest.timeToLive )
			self.guardSpawners[spawner] = (spawnedGuards, pendingGuards + numberOfGuards)
	...
	def _initiateGuardRemoval( self, controllerID ):
		...
		guardsToRemove = min( guardRemoveRequest.numberOfGuards, totalSpawnedGuards )

		for spawner, (spawnedGuards, pendingGuards) in self.guardSpawners.items():
			spawner.removeGuards( guardsToRemove * ( float( spawnedGuards ) / totalSpawnedGuards ) )
	...
	def spawnGuards( self, numberOfGuards, spawnDuration, timeToLive ):
		if numberOfGuards > 0:
			...
			timer = self.addTimer( 0, frequency, GuardSpawner.LOCAL_SPAWNING_TIMER )
	...
	def removeGuards( self, numberOfGuards ):
		for guard in random.sample( self.guardList, min( numberOfGuards, len( self.guardList ) ) ):
			guard.destroyGuard()
...
class GuardSpawnRequest:
	def __init__( self, numberOfGuards, maxGuards, spawnDuration, timeToLive ):
		...
		self.maxGuards = min( maxGuards, GuardSpawnerData.SPAWNED_GUARDS_POPULATION_LIMIT )
		self.spawnDuration = spawnDuration
		self.timeToLive = timeToLive


class LocalGuardSpawnRequest( GuardSpawnRequest ):
	def __init__( self, targetGuardSpawner, numberOfGuards, maxGuards, spawnDuration, timeToLive ):
		GuardSpawnRequest.__init__( self, numberOfGuards, maxGuards, spawnDuration, timeToLive )
