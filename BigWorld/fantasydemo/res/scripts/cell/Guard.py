...
#		self.otherClients.scanForTargets(0.0, 0.0, 0.0)
	...
	def moveToPosition(self, position, velocity, maxMove = MAX_MOVE):
		...
		try:
			...
			ERROR_MSG( "Guard.moveToPosition", e )
...
#		self.otherClients.scanForTargets(0.0, 0.0, 0.0)
	...
	def setDestinationAndSpeed( self ):
		...
		if Guard.ENABLE_PATROL_TESTING:
			...
			try:
				...
				e = GuardNavigationException( self.id, self.position, self.nextPatrolNode.position )
				raise e
	...
	def doPatrol(self):
		...
		if (dist > closeEnough):
			...
			if Guard.ENABLE_PATROL_TESTING:
				filteredDest = self.canNavigateTo( self.destination, -1. )
			else:
...
BigWorld.addWatcher( 'script/Guard/patrolVelocity', getPatrolVelocity, setPatrolVelocity )
...
BigWorld.addWatcher( 'script/Guard/marchVelocity', getMarchVelocity, setMarchVelocity )
