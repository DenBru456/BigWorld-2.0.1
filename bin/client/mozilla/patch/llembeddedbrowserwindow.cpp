...
//get screen bounds (Translated based on the root component as we are mapping).
...
PRBool LLEmbeddedBrowserWindow::sendMozillaMouseEvent( PRInt16 eventIn, PRInt16 xPosIn, PRInt16 yPosIn, PRUint32 clickCountIn )
{
	...
	if ( ! rootWidget )
		return PR_FALSE;
	//make flash mouse work
	nsCOMPtr< nsIWidget > usedWidget = rootWidget; 
	std::list< nsCOMPtr< nsIWidget > > widgetList;
	widgetList.push_back(rootWidget );
	while (widgetList.size() > 0)
	{
		nsCOMPtr< nsIWidget > currWidget = *widgetList.begin();
		widgetList.erase(widgetList.begin());
		nsCOMPtr< nsIWidget > checked = currWidget->GetNextSibling();
		if (checked != NULL)
		{
			widgetList.push_back(checked);
		}
		checked = currWidget->GetFirstChild();
		if (checked != NULL)
		{
			widgetList.push_back(checked);
		}
		...
		nsRect aRect;
		getTranslatedScreenBounds(currWidget, rootWidget, aRect);
		if (aRect.Contains(xPosIn, yPosIn)) 
		{
			usedWidget = currWidget;
		}
	}
	//get the grabbing view
	nsIView* grabbingView;
	viewManager->GetMouseEventGrabber(grabbingView);
	if (usedWidget != rootWidget) 
	{
		nsWindowType type;
		usedWidget->GetWindowType(type);
		HWND hwnd = (HWND)usedWidget->GetNativeData(NS_NATIVE_WINDOW);
		nsRect usedWidgetRect;
		getTranslatedScreenBounds(usedWidget, rootWidget, usedWidgetRect);
		PRInt16 clientCoordX = xPosIn - usedWidgetRect.x;
		PRInt16 clientCoordY = yPosIn - usedWidgetRect.y;
		if (eventIn == NS_MOUSE_LEFT_BUTTON_DOWN)
		{	
			DWORD loc = clientCoordX | clientCoordY << 16;
			SendMessage(hwnd, WM_LBUTTONDOWN, MK_LBUTTON, loc);
			SendMessage(hwnd, WM_LBUTTONUP, MK_LBUTTON, loc);
		}
		if (eventIn == NS_MOUSE_RIGHT_BUTTON_DOWN)
		{	
			DWORD loc = clientCoordX | clientCoordY << 16;
			SendMessage(hwnd, WM_LBUTTONDOWN, MK_RBUTTON, loc);
			SendMessage(hwnd, WM_LBUTTONUP, MK_RBUTTON, loc);
		}
	}
	//set the grabbing view back
	PRBool aResult;
	viewManager->GrabMouseEvents(grabbingView, aResult);
